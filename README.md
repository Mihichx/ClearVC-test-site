...
│   ├── 📄 config.php           - [Игнорируется Git] Локальные настройки окружения и режима debug
│   ├── 📄 PDO.php              - Модуль безопасного подключения к БД через PDO
│   └── 📄 route.php            - Строгая карта маршрутов API и страниц сайта
│
...
│   ├── 📄 ErrorHandler.php     - Единый перехватчик исключений и ошибок ядра
│   └── 📄 Router.php           - Системный маршрутизатор (Роутер) со строгой REST-валидацией
│
├── 📁 views/                   - Представления (HTML-верстка и отображение данных)
│   └── 📁 errors/              - Шаблоны системных ошибок
│       ├── 📄 composer_error.php - Ошибка инициализации зависимостей Composer
│       ├── 📄 config_error.php - Ошибка отсутствия файла конфигурации config.php
│       ├── 📄 error.php        - Детальный бело-красно-чёрный дебаггер для разработчика
│       └── 📄 http.php         - Универсальный минималистичный шаблон для HTTP-статусов
│
├── 📁 vendor/                  - Автозагрузчик и сторонние зависимости (управляется Composer)
│
├── 📄 .gitignore               - Список игнорируемых файлов (папка /vendor/ и config.php скрыты)
...
<div class="container my-5">
  ...
  <div class="card shadow-sm mt-4">
    <div class="card-body">
      <p class="lead">
        <?= htmlspecialchars(\$info['description'] ?? 'Контент временно
        недоступен.') ?>
      </p>
    </div>
  </div>
